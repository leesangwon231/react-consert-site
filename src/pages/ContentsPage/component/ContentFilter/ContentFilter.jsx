    ...
    <div className={'ContentsPage_FilterArea'}>
      <div className={'ContentsPage_Filter_Wrapper'}>
        <div className={'ContentsPage_Filter_ButtonArea'}>
          <div className={`ContentsPage_Filter_Button ${visibleFlag ? 'active' : ''}`} onClick={onClickVisible}>
            {visibleFlag ? '닫기' : '필터'}
          </div>
        </div>
      </div>
          ...
          <div className={'ContentsPage_FilterArea_PerState'}>
            ...
            <div className={'ContentsPage_FilterArea_StateInfoWrapper'}>
              {performanceFilterArray[2]?.map((state, index) => (
                <div
                  className={
                    Object.keys(performanceState)[0] === Object.keys(state)[0]
                      ? 'ContentsPage_FilterArea_Flag_active'
                      : 'ContentsPage_FilterArea_Flag'
                  }
                  key={index}
                  onClick={() => onClickPerformanceState(index)}>
          ...
          <div className={'ContentsPage_FilterArea_PerState'}>
            ...
            <div className={'ContentsPage_FilterArea_StateInfoWrapper'}>
              {performanceFilterArray[1]?.map((sortName, index) => (
                <div
                  className={
                    Object.keys(performanceSort)[0] === Object.keys(sortName)[0]
                      ? 'ContentsPage_FilterArea_Flag_active'
                      : 'ContentsPage_FilterArea_Flag'
                  }
                  key={index}
                  onClick={() => onClickPerformanceSort(index)}>
