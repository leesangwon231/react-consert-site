...
const ContentsPage = () => {
  ...
  const [performanceKinds, setPerformanceKinds] = useState(['콘서트', 'CCCD']);

  if (params.category === 'musical') {
    setPerformanceKinds(['뮤지컬', 'GGGA']);
  } else if (params.category === 'classical') {
    setPerformanceKinds(['클래식/무용', 'BBBC']);
  } else if (params.category === 'play') {
    setPerformanceKinds(['연극', 'AAAA']);
  } else if (params.category === 'concert') {
    setPerformanceKinds(['콘서트', 'CCCD']);
  } else {
    setPerformanceKinds(['콘서트', 'CCCD']);
  ...
  const performanceFilter = [ctprvn, sortedState, perFormanceState];

  const performanceFilterArray = performanceFilter?.map((state) =>
